...
gnuplot -persist <<-EOF
    ...
    set y2tics
    set y2label 'Y2-axis'
    set y2range [2000:4300]

    # Determine the number of columns in the CSV file
    n = system(sprintf("awk -F',' 'NR==1{print NF}' %s", '$CSV_FILE'))

    # Plot the first and second columns on y axis and subsequent columns on y2 axis
    plot '$CSV_FILE' using 1:2 title columnheader(2) with linespoints axis x1y1, \
         '$CSV_FILE' using 1:3 title columnheader(3) with linespoints axis x1y1, \
         for [i=4:n] '$CSV_FILE' using 1:i title columnheader(i) with linespoints axis x1y2
EOF
