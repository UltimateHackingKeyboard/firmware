...
#define RX_BUF_SIZE UART_MAX_PAYLOAD_LENGTH + CRC_LEN
...
static bool isCrcValid(const uint8_t* buf, uint16_t len) {
    if (len < CRC_LEN) {
        return false;
    }
...
static void processIncomingByte(uart_parser_t *uartState, uint8_t byte) {
    ...
    switch (byte) {
        ...
        case UartControlByte_End: {
                ...
                if (len >= CRC_LEN && isCrcValid(data, len)) {
...
void appendByte(uart_parser_t *uartState, uint8_t byte) {
    if (uartState->txPosition < UART_MAX_SERIALIZED_MESSAGE_LENGTH) {
...
void UartParser_InitParser(
    ...
    uartState->receiveMessage = false;
    uartState->escaping = false;
