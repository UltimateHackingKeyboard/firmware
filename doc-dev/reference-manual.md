...
- `ifShortcut/ifNotShortcut/ifGesture/ifNotGesture [IFSHORTCUT_OPTIONS]* [KEYID]*` will wait for next keypresses until sufficient number of keys have been pressed. The default (without `timeoutIn` or `cancelIn`) is to wait forever or until release of the original key. If the next keypresses correspond to the provided arguments (KEYIDs), the keypresses are consumed and the condition is performed. Both `if` and `ifNot` versions will consume the keypresses if the full list is matched. E.g., `ifShortcut 090 089 final tapKey C-V; holdKey v`. `noConsume` can be used to keep the remaining keys for further processing.
  - `Shortcut` requires continuous press of keys (e.g., Ctrl+c), sometimes also called *combo*. By default, it timeouts with the activation key release.
  - `Gesture` allows a noncontinuous sequence of keys (e.g., vim's gg). By default, timeouts in 1000 ms since activation.
  - `IFSHORTCUT_OPTIONS`:
