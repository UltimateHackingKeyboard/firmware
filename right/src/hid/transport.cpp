...
// Called by the USB keyboard application when its HID session starts/stops.
// The BLE HID connection state is owned by bt_conn.c (set on GATT configure).
extern "C" void Hid_TransportStateChanged([[maybe_unused]] bool enabled)
{
    UsbState_SetUsbTransportUp(enabled);
#ifdef __ZEPHYR__
    connection_id_t connectionId = DEVICE_IS_UHK80_LEFT ? ConnectionId_UsbHidLeft : ConnectionId_UsbHidRight;
    Connections_SetStateAsync(
        connectionId, enabled ? ConnectionState_Ready : ConnectionState_Disconnected);
#endif
}
