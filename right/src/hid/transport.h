...
// Called by the USB keyboard application when its HID session starts (enabled=true)
// or stops (enabled=false), to propagate USB transport up/down to UsbState and the
// USB HID connection state. BLE HID connection state is owned by bt_conn.c.
void Hid_TransportStateChanged(bool enabled);
