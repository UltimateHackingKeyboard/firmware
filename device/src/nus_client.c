...
static void ble_data_sent(struct bt_nus_client *nus, uint8_t err, const uint8_t *const data, uint16_t len) {
    k_sem_give(&nusBusy);
    if (err) {
        LOG_WRN("Bt: ATT error code: 0x%02X", err);
    }
}
...
static void discovery_complete(struct bt_gatt_dm *dm, void *context) {
    ...
    if (err) {
        LOG_WRN("Could not assign NUS handles (err %d)", err);
        return;
    }
    ...
    if (err) {
        LOG_WRN("Could not subscribe to NUS notifications (err %d)", err);
        return;
    }
    ...
    if (err) {
        LOG_WRN("Could not release the discovery data (err %d)", err);
        return;
    }

    LOG_DBG("NUS connection with %s successfully established", GetPeerStringByConn(nus->conn));
...
static void gatt_discover(struct bt_conn *conn) {
    ...
    if (err) {
        LOG_WRN("could not start the discovery procedure, error code: %d", err);
    }
}
...
static void exchange_func(struct bt_conn *conn, uint8_t err, struct bt_gatt_exchange_params *params) {
    if (err) {
        ...
        LOG_DBG("MTU exchange done with %s", GetPeerStringByConn(conn));
    }
}
...
void NusClient_Init(void) {
    ...
    if (err) {
        LOG_WRN("NUS Client initialization failed (err %d)", err);
        return;
    }
