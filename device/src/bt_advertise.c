...
static const struct bt_data adNusHid[] = {
    AD_NUS_DATA
    AD_HID_DATA
};
...
static const struct bt_data sdNusHid[] = {
    SD_NUS_DATA
    SD_HID_DATA
};
...
void Advertise(uint8_t adv_type) {
    ...
    char adv_type_string[20];
    if (adv_type == (ADVERTISE_NUS | ADVERTISE_HID)) {
        strcpy(adv_type_string, "NUS and HID");
    } else if (adv_type == ADVERTISE_NUS) {
        strcpy(adv_type_string, "NUS");
    } else if (adv_type == ADVERTISE_HID) {
        strcpy(adv_type_string, "HID");
    }
