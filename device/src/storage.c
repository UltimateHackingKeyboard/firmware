...
uint8_t Storage_LaunchTransfer(eeprom_operation_t operation, config_buffer_id_t configBufferId, void (*successCallback))
{
    ...
    int err = flash_area_open(configAreaId, &configArea);
    ...
    if (operation == EepromOperation_Read) {
        err = flash_area_read(configArea, 0, ConfigBufferIdToConfigBuffer(configBufferId)->buffer, configSize);
    } else if (operation == EepromOperation_Write) {
        err = flash_area_write(configArea, 0, ConfigBufferIdToConfigBuffer(configBufferId)->buffer, configSize);
    }
    return err;
}
