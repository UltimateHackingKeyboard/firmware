...
static void connected(struct bt_conn *conn, uint8_t err) {
    ...
    if (
        ...
        printk("Refusing connenction %d\n", connectionId);
        err = bt_conn_disconnect(conn, BT_HCI_ERR_REMOTE_USER_TERM_CONN);
        return;
    }

    if (DEVICE_IS_UHK80_RIGHT && connectionType == ConnectionType_Unknown) {
        connectionId = ConnectionId_NewBtHid;
        connectionType = ConnectionType_NewBtHid;
    }
